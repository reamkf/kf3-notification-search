...
以下は2026年8月8日から9日に確認した本番外部状態の記録であり、現HEADの本番Workerへの反映、現在のCron・R2・Healthchecks.ioの状態を保証しない。共通契約は [ニュース機能共通仕様](./news-spec.md)、定期実行の仕様は [ニュースアーカイブ定期実行更新仕様](./news-archive-scheduled-spec.md)、ページリクエストの仕様は [ニュースページリクエスト仕様](./news-page-request-spec.md) を参照する。CPU基準の解消、エラー経路、次回実行、ETag条件付き取得の本番確認は未完了である。
